...
// Reconstruit le contrat OpenAPI de BFF Message à partir du paquet @mairie360/bff-message-openapi installé
...
// Même lecteur que Login_Web_Service/scripts/orval-contract.mjs (portage ESM de
// BFFs/*/tests/support/orval-contract.ts), étendu aux types littéraux et aux corps multipart.
//
...
//   (ex. GetMessagingBootstrap401) documentent un statut précis ;
...
export const PACKAGE_NAME = '@mairie360/bff-message-openapi';
...
function readOperation(operationId, fn, models) {
  ...
  // La fabrique `getBffMessage` et les helpers `getXUrl` n'ont pas ce type de retour.
  ...
  // Corps multipart : orval construit un `FormData` local à partir du paramètre `<operationId>Body`.
  const multipart = bodyName !== undefined && !fnParameters.has(bodyName) && /new FormData\(\)/.test(fn.body.getText());
  const bodyParameter = multipart ? fnParameters.get(`${operationId}Body`) : bodyName ? fnParameters.get(bodyName) : undefined;
  ...
  if (bodyParameter?.type) {
    ...
    operation.requestBody = { required: !bodyParameter.questionToken, content: { [contentType]: { schema: typeSchema(bodyParameter.type) } } };
  }
...
function typeSchema(node) {
  ...
  if (ts.isLiteralTypeNode(node)) {
    ...
    if (node.literal.kind === ts.SyntaxKind.TrueKeyword) return { type: 'boolean', enum: [true] };
    if (node.literal.kind === ts.SyntaxKind.FalseKeyword) return { type: 'boolean', enum: [false] };
